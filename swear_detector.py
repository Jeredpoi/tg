...
# Пары (предлог, слово) которые НЕ являются матом.
# "суку/суке" = предложный падеж "сук" (ветка) после пространственных предлогов.
# "сука/суки" после них — тоже чаще "ветка" чем мат.
_PREPOSITIONS = frozenset([
    "на", "в", "по", "за", "при", "под", "над", "у", "от", "до", "из",
    "через", "между", "перед", "около", "вокруг", "с",
])
# Формы слова "сук" (ветка), которые омонимичны матерным
_SUK_BRANCH_FORMS = frozenset(["сука", "суку", "суке", "суки", "сукой"])
...
def _count_swears(text: str) -> int:
    ...
    words = re.findall(r'[а-яё]+', clean)
    for i, word in enumerate(words):
        # Проверяем омонимию "сук" (ветка): формы "суку/суке/сука/суки"
        # после предлога — скорее всего невинный контекст
        if word in _SUK_BRANCH_FORMS:
            prev = words[i - 1] if i > 0 else ""
            if prev in _PREPOSITIONS:
                continue   # пропускаем — "на суку", "за сукой дерева" и т.п.
...
SWEAR_RESPONSES = [
    ...
    "Ну и словечки у тебя, {name}...",
    "{name}, ты так со всеми или только с нами?",
    "Клавиатура у {name} явно б/у от сапожника",
    "Мать слышит, {name}. И я слышу",
    "{name}, курсы по риторике — не слышал?",
    "Статистика {name} по матам стремительно растёт",
    "Папа, {name} опять матерится!",
    "{name}, а в детстве мама так же не слышала?",
    "Воспитание выдаёт себя, {name}",
    "Тихо, {name}, тихо. Дыши",
    "{name} снова открыл рот и закрыл все вопросы",
    "Слушаю и записываю, {name}",
    "Ну-ну, {name}. Продолжай-продолжай",
    "Где-то плачет филолог, {name}",
    "Ой, {name}. Прям ой",
    "Классика от {name}. Снова",
    "Запишу в личное дело, {name}. Уже пятая пометка",
    "Батя слышит тебя, {name}. И молчит. Пока",
    "{name}, слово — не воробей, но у тебя явно птицеферма",
    "{name}, это ты случайно или специально?",
    "Зафиксировано, {name}. Продолжай в том же духе",
]

FLOOD_RESPONSES = [
    "{name}, выдохни. Мы никуда не денемся",
    "Смотрю, {name} сегодня в ударе",
    "{name} пишет как на экзамене — будто время заканчивается",
    "Чат выдохся, а {name} нет",
    "{name}, там все спят уже, только ты не знаешь",
    "Батя следит за {name}. Молча",
    "{name} монополизировал чат. Это не вопрос",
    "У {name} видимо много чего накопилось",
]
